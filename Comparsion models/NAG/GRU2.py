...
optimizer = torch.optim.SGD(model.parameters(), lr=0.08, momentum=0.98, nesterov=True)
...
# 计算评价指标
results = {
    "GRU Training": [assessment.RMSE(outputT, pre_train), assessment.MAE(outputT, pre_train), assessment.MAPE(outputT, pre_train), assessment.R2(outputT, pre_train)],
    "GRU Validation": [assessment.RMSE(outputV, pre_valid), assessment.MAE(outputV, pre_valid), assessment.MAPE(outputV, pre_valid), assessment.R2(outputV, pre_valid)],
    "GRU Test": [assessment.RMSE(outputTEST, pre_test), assessment.MAE(outputTEST, pre_test), assessment.MAPE(outputTEST, pre_test), assessment.R2(outputTEST, pre_test)],
    }
print(pre_train.shape)
print(pre_valid.shape)
