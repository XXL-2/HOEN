...
class CosineAnnealingRestartLR(LRScheduler):
    ...
    def get_lr(self):
        if self.last_epoch < swa_start:
            return [0.009 for _ in self.base_lrs]
        else:
...
# 计算评价指标
results = {
    "SWA-LSTM Training": [assessment.RMSE(outputT, pre_train), assessment.MAE(outputT, pre_train), assessment.MAPE(outputT, pre_train), assessment.R2(outputT, pre_train)],
    "SWA-LSTM Validation": [assessment.RMSE(outputV, pre_valid), assessment.MAE(outputV, pre_valid), assessment.MAPE(outputV, pre_valid), assessment.R2(outputV, pre_valid)],
    "SWA-LSTM Test": [assessment.RMSE(outputTEST, pre_test), assessment.MAE(outputTEST, pre_test), assessment.MAPE(outputTEST, pre_test), assessment.R2(outputTEST, pre_test)],
    }
